# -*- coding: utf-8 -*-
"""week4_updated_two_layer_net.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1vP8J93qg0ZJ7_h6oCW4ytLNjSVq1EINj
...
def nn_loss_part2(params, X, y=None, reg=0.0):
    ...
    H, C = W2.shape
    relu = torch.nn.ReLU()
    #XdotW1 = X.mm(W1) + b1                    # 1) (N,D)mm(D,H) + (H,) = (N,H)
    #hidden = relu(XdotW1)                     # 2) (N,H)
    f = h1.mm(W2) + b2                    # 3) (N,H)mm(H,C) + (C,) =  (N,C)
    f_max = f.max(dim=1, keepdim=True).values # 3.1) Numeric instability.
    f -= f_max                                # 3.2)
    f_exp = f.exp()                           # 4) (N,C)
    f_exp_s = f_exp.sum(dim=1, keepdim=True)  # 5) (N,1)
    f_exp_s_l = f_exp_s.log()                 # 6) (N,1)
    f_exp_s_l_s = f_exp_s_l.sum()             # 7) (1,)
    f_exp_s_l_s_div = f_exp_s_l_s / float(N)  # 8) (1,)
    y_m = torch.zeros((N,C)).to(W1)           # (N,C). y matrix
    y_m[range(N), y] = 1
    f_y_m = (f*y_m)                           # 9) (N,C)
    f_y_m_s = f_y_m.sum()                     # 9.1) (1,)
    f_y_m_s_div = f_y_m_s / -float(N)         # 10) (1,)
    regW1pow2sum = W1.pow(2).sum() * 2 * reg   # 11) ()
    regW2pow2sum = W2.pow(2).sum() * 2 * reg  # 12)
    rprod = regW1pow2sum + regW2pow2sum  # 13)
    loss = f_y_m_s_div + f_exp_s_l_s_div + rprod   # 14) 

    dloss = 1.0
    # 14)
    df_y_m_s_div = dloss
    df_exp_s_l_s_div = dloss
    drprod = dloss
    # 13)
    d2regW1pow2sum = drprod
    d2regW2pow2sum = drprod
    # 12)
    dW2 = d2regW2pow2sum * 2 * reg * torch.ones_like(W2) * 2 * W2  # (H,C)
    # 11)
    dW1 = d2regW1pow2sum * 2 * reg * torch.ones_like(W1) * 2 * W1  # (D,H)
    # 10)
    df_y_m_s = df_exp_s_l_s_div * (1.0 / -float(N))  # (1,)
    # 9.1)
    df_y_m = torch.ones_like(f_y_m) * df_y_m_s  # (N,C)
    # 9
    df = y_m * df_y_m  # (N,C) * (N,C)
    # 8
    df_exp_s_l_s = df_exp_s_l_s_div * (1.0 / float(N))  # (1,)
    # 7 
    df_exp_s_l = torch.ones_like(f_exp_s_l) * df_exp_s_l_s  # (N, 1)
    # 6
    df_exp_s = (1.0 / f_exp_s ) * df_exp_s_l  # (N,1) * (N,1)
    # 5 
    df_exp = df_exp_s.mm(torch.ones((1,C)).to(f_exp))  # (N,C)
    #df_exp = torch.ones_like(f_exp).sum(dim=1, keepdim=True) * df_exp_s  # (N,1)
    # 4
    df = df + f_exp * df_exp  # (N,C) + (N,C) * (N,1) = (N,C)
    # 3.2
    # df = 
    # 3 
    dW2 = dW2 + h1.T.mm(df)  # (H,N) mm (N,C)  -> (H,C)
    db2 = df.T.sum(dim=1)  # (C,N) mm (N,1) = (C, 1)
    # 3
    dhidden = df.mm(W2.T)  # (N,C) x (C,H)-> (N,H)
    # 2
    drelu = torch.where(h1 <= 0, 0, dhidden)  # (N,H)
    # 1
    dW1 = dW1 + X.T.mm(drelu) # (D,N) x (N,H) -> (D,H)
    db1 = drelu.T.sum(dim=1)   #  -> (H,)
