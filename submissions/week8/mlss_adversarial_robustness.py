# -*- coding: utf-8 -*-
"""mlss_Adversarial_Robustness.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1tvRiP5mHqg0RaSYYYqs2Cmr54UZG8NNQ
...
def untargeted_FGSM(x_batch, true_labels, network, normalize, eps=8/255., **kwargs):
  ...
  l = loss_fn(network(normalize(x_batch)), true_labels)
  l.backward()
...
def targeted_FGSM(x_batch, target_labels, network, normalize, eps=8/255., **kwargs):
  ...
  #########################
  # Enter Your Code Here! #
  l = loss_fn(network(normalize(x_batch)), target_labels)
  l.backward()
  x_adv = x_batch - eps * x_batch.grad.sign()
  #########################
...
def normalize_l2(x_batch):
    ...
    #########################
    # Enter Your Code Here! #
    return x_batch / torch.linalg.vector_norm(x_batch, ord=2, dim=(1,2,3))
    #########################
...
def tensor_clamp_l2(x_batch, center, radius):
    ...
    #########################
    # Enter Your Code Here! #
    l2norm = torch.linalg.vector_norm((x_batch - center), ord=2, dim=(1,2,3))
    return torch.where(
        l2norm > radius,
        center + (x_batch - center)/l2norm * radius, # clamp
        x_batch)
...
Example responses: (FILLED BY Artem K on 13 aug 2022)

1. The confidence of typical models is higher than adversarially trained models
  True as I see I could turn answer from 'giant panda' to any class I like with 99% confidence. Example: black widow, 75 class. But 
2. If I make only one step to train then it doesn't construct x_adv enough to fool the adv untrained network. Tried with `num_steps=1`. It outputs: "cocker spaniel" with 9.2% confidence.
3. I couldn't increase confidence for the adv trained model for the panda example as above. I tried increasing `esp`, step_size, num_steps. These led to the corruption of image but no change to prediction confidence. So the model seems robust to that type of attack.
...
def adversarial_accuracy(x_batch, true_labels, network, normalize, num_steps=10, step_size=0.01, eps=8/255.):
    ...
    #########################
    # Enter Your Code Here! #
    #logits = network(x_adv) # This gives accuracy 48% but it won't change if num_steps increased.
    logits = network(normalize(x_adv))  # This gives accuracy that is changed over num of steps but it is lower, around 25%.
    N, C, W, H = x_batch.shape
    accuracy = (logits.argmax(dim=1) == true_labels).sum() / N
...
for num_steps in steps_arr:
  accuracy = adversarial_accuracy(
      ...
      num_steps=num_steps)
  print('steps, accuracy:', num_steps, accuracy)
  stats.append(accuracy)

# plot the loss history
plt.plot(steps_arr, stats, 'o')
plt.xlabel('steps')
plt.ylabel('accuracy')
plt.show()
